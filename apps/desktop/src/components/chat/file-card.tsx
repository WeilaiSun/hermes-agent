...
import { openPreview as openPreviewInStore } from '@/store/preview'
...
export function FileCard({ path }: FileCardProps) {
  ...
  const openPreview = useCallback(async () => {
    ...
    if (target) {
      openPreviewInStore({ ...target, source: 'file-card' }, 'file-browser')
    }
        ...
        {(fileExtension(path) === 'html' || fileExtension(path) === 'htm') && (
          <ContextMenuItem
            onClick={() => {
              const target = localPreviewTarget(path)
              if (target) {
                openPreviewInStore({ ...target, renderMode: 'source', source: 'file-card' }, 'file-browser')
              }
            }}
          >
