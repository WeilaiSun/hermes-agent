...
def _bash_safe_path(path: str) -> str:
    ...
    Native ``C:\\Users\\x`` / ``C:/Users/x`` are kept as drive-qualified
    forward-slash form (``C:/Users/x``) rather than rewritten to MSYS
    ``/c/Users/x``: with ``MSYS_NO_PATHCONV=1`` (set by this module) Git
    Bash passes ``C:/...`` through untouched to Windows-native programs
    (ripgrep, node, git), while MSYS builtins/coreutils also accept it —
    so both sides can consume the same string.  (The old ``/c/...``
    rewrite made `search_files` fail on any absolute path: `rg` is a
    native exe and cannot resolve ``/f/...``, surfacing as 0 matches or
    ``rg: /f/...: 系统找不到指定的路径 (os error 3)``.)

    Mixed MSYS leftovers (``/c/Users\\Alexander\\Documents``) get
    backslashes normalized so bash does not eat ``\\U`` and trip the
    ``Directory \\drivers\\etc`` failure class. No-op off Windows and
    for empty input.
    """
